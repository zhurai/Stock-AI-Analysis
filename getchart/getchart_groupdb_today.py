...
def main():
    ...
    if not os.path.exists(utils.getroot()+localconfig['localfoldergz']+latestdate):
        os.makedirs(utils.getroot()+localconfig['localfoldergz']+latestdate)
    if not os.path.exists(utils.getroot()+localconfig['localfolder']+latestdate):
        os.makedirs(utils.getroot()+localconfig['localfolder']+latestdate)

    # download all the files to input/gzip/yyyymmdd
    for item in datedir:
        download_file_from_google_drive(item['id'], utils.getroot()+localconfig['localfoldergz']+latestdate+"/"+item['name'])

        # ungzip the csv.gz in input/gzip/yyyymmdd/*.csv.gz to input/yyyymmdd/*.csv
        with gzip.open(utils.getroot()+localconfig['localfoldergz']+latestdate+"/"+item['name'],'rt') as f:
            with open(utils.getroot()+localconfig['localfolder']+latestdate+"/"+item['name'][:-3],'w') as f2:
                for line in f:
                    f2.write(line)
