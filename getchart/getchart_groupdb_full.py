...
def main():
    ...
    for index,item in enumerate(csvidlist):
        ...
        download_file_from_google_drive(item, utils.getroot()+localconfig['localfoldergz']+filename[index])

        # ungzip the csv.gz in input/gzip/*.csv.gz to input/*.csv
        with gzip.open(utils.getroot()+localconfig['localfoldergz']+filename[index],'rt') as f:
            with open(utils.getroot()+localconfig['localfolder']+filename[index][:-3],'w') as f2:
                for line in f:
                    f2.write(line)
